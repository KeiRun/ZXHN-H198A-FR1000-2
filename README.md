...
Кастомная прошивка OpenWrt для роутеров ZTE E8820S / FR1000-2 / ZXHN H198A с поддержкой **SPI NOR flash** (W25Q512JV 64MB), а также материалы для модификации и восстановления.

> **ПРЕДУПРЕЖДЕНИЕ:** Сборка OpenWrt в этом репозитории — ТОЛЬКО для устройств, аппаратно модифицированных для работы с SPI NOR flash вместо стоковой NAND. Если на вашем устройстве ещё стоит родной чип NAND, используйте оригинальную цель `ZTE_E8820S`. Для возврата к NAND смотрите материалы в папке `boot_mode_resistors_nand_nor/`.
...
- `kmod-mt7603` (2.4 GHz Wi-Fi)
- `kmod-mt76x2` (5 GHz Wi-Fi)
- `kmod-usb3`
- LuCI (веб-интерфейс) с русским языком и терминалом
- Podkop, B4 (см. их репозитории)
- Все необходимые зависимости (curl, jq, kmod-nft-tproxy и др.)
